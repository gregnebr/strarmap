...
public class SpatialConverter {
	...
	public static Coordinate toEquatorial(Point pt) {
		return toEquatorial(pt.getCoordinate());
	}

	public static Coordinate toEquatorial(Coordinate c) {
		double ra = (-c.x) / 15.0 + 12.0; // convert to hours then add 12
		double rightDecl = c.y;
		return new Coordinate(ra, rightDecl);
	}

	public static BufferedImage render(Collection<StarData> data, int width,
